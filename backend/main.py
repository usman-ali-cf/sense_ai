# Update the ChatRequest model and chat endpoint to match the new payload format
...
async def chat(request: ChatRequest):
    # Extract session ID and message from the new payload format
    ...
    print(f"Received chat request with session_id: {session_id}")
    ...
    if session_id not in chat_sessions:
        raise HTTPException(status_code=404, detail=f"Chat session not found for ID: {session_id}")
    ...
    try:
        ...
        response = retrieval_chain.invoke({
            "question": message,
            "chat_history": []  # No history in this format
        })
...
async def create_session(doc_id: str):
    ...
    try:
        ...
        return {"session_id": doc_id, "status": "created"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
